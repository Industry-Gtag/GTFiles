...
public class SIGadgetTentacleArm : SIGadget, ICallBack, IEnergyGadget
{
	...
	protected override void OnUpdateAuthority(float dt)
	{
		...
		bool num2 = Physics.SphereCast(new Ray(knownSafePosition, idealClawPosition - knownSafePosition), num, out var hitInfo2, (idealClawPosition - knownSafePosition).magnitude, worldCollisionLayers);
	...
	protected override void OnUpdateRemote(float dt)
	{
		...
		float num = 0.15f;
		bool num2 = Physics.SphereCast(new Ray(knownSafePosition, idealClawPosition - knownSafePosition), num, out var hitInfo, (idealClawPosition - knownSafePosition).magnitude, worldCollisionLayers);
		Vector3 axis = Quaternion.AngleAxis(Time.time * 180f, Vector3.forward) * Vector3.up;
		Vector3 vector = idealClawPosition;
