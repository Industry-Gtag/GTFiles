...
public class GameEntityManager : NetworkComponent, IRequestableOwnershipGuardCallbacks, ITickSystemTick
{
	...
	private void RegisterSingleScenePlacedEntity(GameEntity entity)
	{
		...
		int num = ((!entity.TryGetComponent<XSceneRefTarget>(out var component) || component.UniqueID <= 0) ? ComputeNetIdFromHierarchyForCustomMaps(entity.transform) : NetIdFromXSceneRefId(component.UniqueID));
	...
	public void JoinWithItemsRPC(byte[] stateData, int[] netIds, int joiningActorNum, PhotonMessageInfo info)
	{
		...
		bool flag = playerZoneJoinTimes.TryGetValue(joiningActorNum, out var value) && Time.unscaledTime - value < 10f;
		bool num = GamePlayer.TryGetGamePlayer(joiningActorNum, out var joiningPlayer);
	...
	private void SetZoneState(ZoneState newState)
	{
		...
		if (flag4 != flag)
		{
			OnZoneActiveChanged?.Invoke(flag4);
		}
	}
	...
	{
		OnAuthorityChanged?.Invoke(fromPlayer, toPlayer);
