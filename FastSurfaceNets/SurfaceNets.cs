...
namespace FastSurfaceNets
{
	...
	{
		// Token: 0x06007CC7 RID: 31943 RVA: 0x0028C2AC File Offset: 0x0028A4AC
		public static void Generate(float[] sdf, int3 shape, int3 min, int3 max, SurfaceNetsBuffer output)
		{
			...
			for (int i = min.z; i < max.z; i++)
			{
				for (int j = min.y; j < max.y; j++)
				{
					...
					{
						float3 @float;
						if (SurfaceNets.EstimateSurfaceInCube(sdf, shape, new int3(k, j, i), num4, num2, x, num3, output, out @float))
						{
							...
							output.Positions.Add(new float3((float)k, (float)j, (float)i) + @float);
						}
						k++;
						num4++;
					}
		...
		// Token: 0x06007CC8 RID: 31944 RVA: 0x0028C41C File Offset: 0x0028A61C
		private static bool EstimateSurfaceInCube(float[] sdf, int3 shape, int3 voxel, int cubeMinStride, int strideX, int strideY, int strideZ, SurfaceNetsBuffer output, out float3 centroid)
		{
			...
			foreach (int2 int2 in SurfaceNets.CubeEdges)
			{
				...
				{
					num4++;
					@float += SurfaceNets.EdgeIntersection(int2.x, int2.y, num5, num6);
				}
			}
			centroid = @float / (float)num4;
			output.Normals.Add(float3.zero);
			return true;
		}

		// Token: 0x06007CC9 RID: 31945 RVA: 0x0028C538 File Offset: 0x0028A738
		...
		// Token: 0x06007CCA RID: 31946 RVA: 0x0028C578 File Offset: 0x0028A778
		private static float3 CentralDifferenceGradient(float[] sdf, int3 shape, int3 v, int sx, int sy, int sz)
		{
			...
			float num7 = sdf[num2 + v.y * sy + v.z * sz] - sdf[num + v.y * sy + v.z * sz];
			float num8 = sdf[v.x + num4 * sy + v.z * sz] - sdf[v.x + num3 * sy + v.z * sz];
			float num9 = sdf[v.x + v.y * sy + num6 * sz] - sdf[v.x + v.y * sy + num5 * sz];
			return new float3(num7, num8, num9);
		}

		// Token: 0x06007CCB RID: 31947 RVA: 0x0028C694 File Offset: 0x0028A894
		...
		// Token: 0x06007CCC RID: 31948 RVA: 0x0028C794 File Offset: 0x0028A994
		private static void MaybeQuad(float[] sdf, SurfaceNetsBuffer b, int p1, int p2, int strideB, int strideC)
		{
			...
			float3 @float = b.Positions[num3];
			float3 float2 = b.Positions[num4];
			float3 float3 = b.Positions[num5];
			float3 float4 = b.Positions[num6];
			if (math.lengthsq(@float - float4) < math.lengthsq(float2 - float3))
			{
				if (flag)
				{
					b.Indices.AddRange(new int[] { num3, num6, num4, num3, num5, num6 });
					return;
				}
				b.Indices.AddRange(new int[] { num3, num4, num6, num3, num6, num5 });
				return;
			}
			else
			{
				if (flag)
				{
					b.Indices.AddRange(new int[] { num4, num5, num6, num4, num3, num5 });
					return;
				}
				b.Indices.AddRange(new int[] { num4, num6, num5, num4, num5, num3 });
				return;
			}
		}

		// Token: 0x06007CCD RID: 31949 RVA: 0x0028C930 File Offset: 0x0028AB30
		private static void AccumulateNormals(SurfaceNetsBuffer b)
		{
			for (int i = 0; i < b.Indices.Count; i += 3)
			{
				...
				float3 @float = b.Positions[num];
				float3 float2 = b.Positions[num2];
				float3 float3 = b.Positions[num3];
				float3 float4 = math.cross(float2 - @float, float3 - @float);
				List<float3> list = b.Normals;
				int num4 = num;
				list[num4] += float4;
				list = b.Normals;
				num4 = num2;
